...
jclass _getClassID(const char *className) {
    ...
    jclass _clazz = (jclass) env->CallObjectMethod(JniHelper::classloader,
                                                   ...
                                                   _jstrClassName);
    if (nullptr == _clazz) {
        JniLOGE("Classloader failed to find class of %s", className);
        env->ExceptionDescribe();
        env->ExceptionClear();
    }
...
void _detachCurrentThread(void* addr) {
    ...
    //JniLOGE("#######_detachCurrentThread %ld %ld ret:%d",(long)pthread_self(),(long)addr,ret);
}
...
JavaVM* JniHelper::getJavaVM() {
    ...
    JniLOGD("JniHelper::getJavaVM(), pthread_self() = %ld", thisthread);
    return _psJavaVM;
}
...
void JniHelper::setJavaVM(JavaVM *javaVM) {
    pthread_t thisthread = pthread_self();
    JniLOGD("JniHelper::setJavaVM(%p), pthread_self() = %ld", javaVM, thisthread);
    _psJavaVM = javaVM;
...
JNIEnv* JniHelper::cacheEnv(JavaVM* jvm) {
    ...
    case JNI_OK :
        // Success!
        //JniLOGE("#######pthread_setspecific JNI_OK %ld",(long)pthread_self());
    ...
    case JNI_EDETACHED :
        ...
        {
            JniLOGE("Failed to get the environment using AttachCurrentThread()");
            return nullptr;
        } else {
            // Success : Attached and obtained JNIEnv!
            //JniLOGE("#######pthread_setspecific JNI_EDETACHED %ld",(long)pthread_self());
            pthread_setspecific(g_key, _env);
            return _env;
        }
    ...
    case JNI_EVERSION :
        ...
        JniLOGE("JNI interface version 1.4 not supported");
    default :
        JniLOGE("Failed to get the environment using GetEnv()");
        return nullptr;
    }
}
...
bool JniHelper::getStaticMethodInfo(JniMethodInfo &methodinfo,
    ...
    JNIEnv *env = JniHelper::getEnv();
    if (!env) {
        JniLOGE("Failed to get JNIEnv");
        return false;
    }

    jclass classID = _getClassID(className);
    if (! classID) {
        JniLOGE("Failed to find class %s", className);
        env->ExceptionClear();
        return false;
    }
    ...
    if (! methodID) {
        JniLOGE("Failed to find static method id of %s", methodName);
        env->ExceptionClear();
        return false;
    }
...
bool JniHelper::getMethodInfo_DefaultClassLoader(JniMethodInfo &methodinfo,
    ...
    if ((nullptr == className) ||
            (nullptr == methodName) ||
            (nullptr == paramCode)) {
         JniLOGE("getmethod para err");
        return false;
    }

    JNIEnv *env = JniHelper::getEnv();
    if (!env) {
        JniLOGE("JniHelper::getEnv err");
        return false;
    }
    ...
    if (! methodID) {
        JniLOGE("Failed to find method id of %s", methodName);
        env->ExceptionClear();
        return false;
    }
...
jclass JniHelper::findClass(const char *className){
    ...
    if(clazz==nullptr){
        env->ExceptionClear();
        //JniLOGE("_getClassID %ld",(long)clazz);
        clazz=_getClassID(className);
        if(env->ExceptionCheck()){
            //JniLOGE("ExceptionCheck %ld",(long)clazz);
            clazz=nullptr;
            env->ExceptionDescribe();
            env->ExceptionClear();
        }
...
bool JniHelper::getMethodInfo(JniMethodInfo &methodinfo,
                              ...
                              const char *methodName,
                              const char *paramCode){
    //JniLOGE("----JniHelper::getMethodInfo----");
    if ((nullptr == jobj) ||
            (nullptr == methodName) ||
            (nullptr == paramCode)) {
         JniLOGE("getMethodInfo para err");
        return false;
    }
